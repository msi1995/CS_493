...
/*
 * Route to delete a review, only if you own it.
 */
router.delete('/:id', requireAuthentication, async (req, res, next) => {

  const userInfo = await getUserById(req.user)
  const reviewInfo = await getReviewById(req.params.id)
  if(userInfo.admin == 1 || req.user == reviewInfo.userid ){
    try {
    ...
    res.status(403).send({
      error: "You are not the owner of this review and cannot delete it."
    })
  }
});
