...
async function getBusinessDetailsById(id) {
  ...
  if(business.photos){
    for(i=0; i < business.photos.length; i++){
    delete business.photos[i].length
    delete business.photos[i].chunkSize
    delete business.photos[i].uploadDate
    delete business.photos[i].filename
    delete business.photos[i].md5
    }
    }
  return business;
}
